                ...
                if os.path.exists(icon_path):
                    ...
                    if not pixmap.isNull():
                        ...
                        pixmap = pixmap.scaled(
                            cls._icon_size, Qt.KeepAspectRatio, Qt.SmoothTransformation
                        )
...
class DateTimeDialog(QDialog):
    ...
    def validate_and_accept(self):
        ...
        selected_datetime = (
            f"{selected_date.toString('yyyy-MM-dd')} {selected_time.toString('HH:mm')}"
        )
        ...
        if not self.is_start_time and self.deadline:
            ...
            if self.deadline not in ["Start time", ""]:
                ...
                if selected_date < start_date or (
                    selected_date == start_date and selected_time <= start_time
                ):
                    QMessageBox.warning(
                        self,
                        "Validation Error",
...
class AddTaskWidget(QtWidgets.QWidget):
    ...
    def _setup_buttons(self, main_layout):
        ...
        self.StartTime = self.create_button(
            "StartTime", "Calender_icon.png", "Start time", icon_manager
        )
        self.Deadline = self.create_button(
            "Deadline", "Calender_icon.png", "Deadline", icon_manager
        )
        self.Priority = self.create_button(
            "Priority", "Flag_icon.png", "Priority", icon_manager
        )
        self.Reminder = self.create_button(
            "Reminder", "Reminder_icon.png", "Reminder", icon_manager
        )
        ...
        self.StartTime.clicked.connect(
            lambda: self.show_datetime_dialog(self.StartTime)
        )
        self.Deadline.clicked.connect(lambda: self.show_datetime_dialog(self.Deadline))

        # Create dropdown menus
        self.priority_dropdown = self.create_dropdown_menu(
            self.Priority, ["None", "Low", "Medium", "High"], icon_manager
        )
        self.reminder_dropdown = self.create_dropdown_menu(
            self.Reminder,
            [
                "None",
                "5 minutes before",
        ...
        self.Priority.clicked.connect(
            lambda: self.toggle_dropdown(self.priority_dropdown, self.Priority)
        )
        self.Reminder.clicked.connect(
            lambda: self.toggle_dropdown(self.reminder_dropdown, self.Reminder)
        )
        ...
        if button == self.Priority:
            ...
            for item in items:
                ...
                action.triggered.connect(
                    lambda checked, text=item: self.handle_dropdown_selection(
                        button, text
                    )
                )
        else:
    ...
    def validate_start_time(self):
        if self.StartTime.text() != "Start time" and self.Deadline.text() != "Deadline":
            start_date = QDate.fromString(
                self.StartTime.text().split(" ")[0], "yyyy-MM-dd"
            )
            start_time = QTime.fromString(self.StartTime.text().split(" ")[1], "HH:mm")
            deadline_date = QDate.fromString(
                self.Deadline.text().split(" ")[0], "yyyy-MM-dd"
            )
            deadline_time = QTime.fromString(
                self.Deadline.text().split(" ")[1], "HH:mm"
            )
    ...
    def validate_deadline(self):
        ...
        if deadline_date < start_date or (
            deadline_date == start_date and deadline_time <= start_time
        ):
            QMessageBox.warning(
                self,
                "Validation Error",
    ...
    def set_task_data(self, task_data):
        """Set the widget fields from a task_data dictionary."""
        ...
        self.Priority.setText(task_data.get("priority", "Priority"))
        self.Reminder.setText(task_data.get("reminder", "Reminder"))
